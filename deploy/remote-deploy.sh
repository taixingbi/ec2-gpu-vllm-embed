...
echo "=== Deploy vLLM embeddings ==="
sudo docker compose pull -q
echo "Pull done, starting containers..."
sudo docker compose up -d

echo "=== Waiting for vLLM embeddings (models may take 5-15 min to load) ==="
sudo docker ps -a
for i in $(seq 1 15); do
  if curl -sf http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
    echo "vLLM embeddings ready"
    break
  fi
  echo "Still waiting... $((i))min"
  sudo docker ps -a --format "table {{.Names}}\t{{.Status}}" 2>/dev/null || true
  if [ $i -eq 15 ]; then
    echo "Timeout (15 min). Container status:"
    sudo docker ps -a
    sudo docker logs vllm_embed --tail 50 2>&1 || true
    exit 1
  fi
  sleep 60
done
...
echo "=== Done ==="
sudo docker logs vllm_embed --tail 5 2>&1 || true
